...
<?php include '../studheader2.php';?>
    ...
    <br />
		<div class="row">
			<div class="col-sm">
				<button type="button" id="leave" class="btn btn-primary" onclick="showLeaveRequests('leave')">Leave Status</button>
			</div>
			
			<div class="col-sm">
				<button type="button" id="complaint" class="btn btn-primary" onclick="showComplaintRequests('complaint')">Complaint Status</button>
			</div>

      <div class="col-sm">
				<button type="button" id="mess" class="btn btn-primary" onclick="showMessMenu('mess')">Mess Menu</button>
			</div>
		</div>
		<div class="row mt-3">
			<div class="col-sm">
	...
	<script>
		function showLeaveRequests(status) {
			$.ajax({
				url: "lstatus.php",
				type: "POST",
				data: {status: status},
				success: function(data) {
					$("#requestsTable tbody").html(data);
				}
			});
		}

		function showComplaintRequests(status) {
			$.ajax({
				url: "cstatus.php",
				type: "POST",
				data: {status: status},
...
recognition.onend = () => {
  ...
  else if (speechInput.includes("leave status")) 
  {
    console.log('Detected keyword: ' + speechInput.toLowerCase());
    var Btn = document.getElementById("leave");
    Btn.click();
  }
  else if (speechInput.includes("complaint status")) 
  {
    console.log('Detected keyword: ' + speechInput.toLowerCase());
    var Btn = document.getElementById("complaint");
    Btn.click();
  }
  else if (speechInput.includes("menu")) 
  {
    console.log('Detected keyword: ' + speechInput.toLowerCase());
    var Btn = document.getElementById("mess");
    Btn.click();
  }
