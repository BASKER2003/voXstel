...
(min-device-width: 768px) and (max-device-width: 1024px)  {
  ...
  <body>
    ...
    <?php
      ...
      $sql="SELECT DISTINCT * FROM complaints where `block`='$warden_hostel';";
      ...
      function approve() 
      {
        ...
        $regno1=$_POST['id'];
        $sql1="UPDATE `complaints` SET `status`='resolved' where id='$regno1'";
        $query1=mysqli_query($conn,$sql1);
        if($query1)
        {
          echo "Complaint will be resolved soon!!!";
        }
        else
        {
      ...
      function reject() 
      {
        $conn= mysqli_connect('localhost','root','','hms') or die("Connection failed:" .mysqli_connect_error());
        $regno1=$_POST['id'];
        $sql1="UPDATE `complaints` SET `status`='rejected' where id='$regno1'";
        $query1=mysqli_query($conn,$sql1);
        if($query1)
        {
          echo "This complaint is unnecessay!!!";
        }
        else
        {
          echo "An error occurred";
          ...
          <?php while($rows=mysqli_fetch_assoc($query)) { ?>
            <tr>
              ...
              <?php if($rows['status']=="Pending") { ?>
                <td>
                  <input type="submit" name="approve" class="approvebtn" value="Resolve">
